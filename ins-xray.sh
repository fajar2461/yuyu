...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/fajar2461/yuyu/main/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/fajar2461/yuyu/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/fajar2461/yuyu/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
wget -q -O /usr/bin/bbr "https://raw.githubusercontent.com/fajar2461/yuyu/main/bbr.sh"
chmod +x /usr/bin/bbr
bbr >/dev/null 2>&1
